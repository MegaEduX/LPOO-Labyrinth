...
public class Eagle extends ItemizablePiece {
    ...
    public Boolean move(Coordinate c1) throws Exception {
        ...
        try {
            ...
            if (_position.x < c1.x) {
                if (_position.y < c1.y) {
                    return (super.move(Board.Direction.DOWN) && super.move(Board.Direction.RIGHT));
                }
                if (_position.y > c1.y) {
                    return (super.move(Board.Direction.UP) && super.move(Board.Direction.RIGHT));
                }

            }
            if (_position.x > c1.x) {
                if (_position.y < c1.y) {
                    return (super.move(Board.Direction.DOWN) && super.move(Board.Direction.LEFT));
                }
                if (_position.y > c1.y) {
                    return (super.move(Board.Direction.UP) && super.move(Board.Direction.LEFT));
                }
            }
            // Found the Sword
            _item = true;
